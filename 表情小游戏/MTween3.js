...
function MTween(obj,attrObj,duration,fx,callBack){
	...
	*/
	var newObj = {}
	for( var attr in attrObj ){//attr是属性，attrObj是对象
		newObj[attr] = {};
	}
	...
	var d = duration;
	fx = fx || "linear";
	clearInterval(obj.timerMT);
	obj.timerMT = setInterval(function (){
		var t = new Date().getTime() - current;
		if( t >= d || onOff){//运动完成或者开关变成true的时候，定时器停止，同时把定时器清空
			clearInterval(obj.timerMT);
			obj.timerMT = null;
			t = d;
			if(onOff){
